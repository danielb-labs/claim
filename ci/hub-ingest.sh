...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --claim-bin)
      ...
      ;;
    *)
      die "unrecognized argument \`$1\` (usage: --hub-url <url> --audience <aud> [--claims-dir <dir>] [--claim-bin <path>])"
      ;;
  esac
done
...
\`permissions: id-token: write\`); cannot request an OIDC token"
  ...
  # The GitHub Actions token endpoint mints a token for the given audience.
  ...
  response="$(curl --silent --show-error --fail-with-body \
    ...
    || die "requesting an OIDC token from the GitHub Actions endpoint failed"
...
# A report that is not valid JSON means the CLI itself failed to produce a report (a
# crash, a missing store) — there is nothing honest to push, so fail loud rather than
# POST garbage the hub would reject anyway.
jq -e . "$report_file" >/dev/null 2>&1 \
  || die "\`$claim_bin check --json\` did not produce a JSON report (exit $check_status); nothing to push"
...
http_code="$(curl --silent --show-error \
  ...
  "$ingest_endpoint")" \
  || die "POST to $ingest_endpoint failed to complete (the hub was unreachable or the request errored)"
...
case "$http_code" in
  2??)
    # Success. Report what the hub recorded so the lane's log shows the outcome.
    accepted="$(printf '%s' "$response_body" | jq -r '.accepted // "?"' 2>/dev/null || echo '?')"
