...
//! exit-code contract is pinned here: 0 when every verb is documented, 1 when one is
//! missing (drift), 2 when a source it reads is absent (broken, never a false pass).
//!
//! Each case builds a throwaway repo skeleton — the script, a real `claim` binary, and
//! a *controlled* `docs/index.html` — and runs the script against it, so the docs the
//! script judges are the test's, not the repo's.
...
/// `claim` binary at `target/debug/claim`, and the given `index.html`. Returns the
/// skeleton root; the script derives its own `repo_root` from the script's location
/// within it, so everything must sit at the real relative paths.
...
/// An `index.html` documenting every current verb — the covered case. Verbs appear as
...
fn all_documented_holds_with_exit_0() {
    ...
    assert_eq!(
        run_script(dir.path()),
        0,
        "a site documenting every verb must hold"
    );
}
