...
//! and `report` (append a verdict an agent reached, with evidence, under its own
//! git identity). It is a thin shell over [`claim_core`] and [`claim_store`]: the
//! protocol wiring lives in [`server`], the tool logic in [`query`] and
//! [`report`], and this entry point only builds the server and serves it over
//! stdio — the transport an MCP client (an agent) connects a subprocess over.
//!
